...
deploy_all_stacks() {
  ...
  if [ ${#failed[@]} -eq 0 ]; then
    # ServicesStack and GatewayStack both depend on BackendStack only
    deploy_stack "citadel-services-$env" || failed+=("services")
    deploy_stack "citadel-gateway-$env"  || failed+=("gateway")
  else
    warn "Skipping services/gateway — backend failed"
  fi
...
show_help() {
  ...
  echo "  --backend-only       Deploy only backend stacks (backend + services + arbiter + gateway)"
